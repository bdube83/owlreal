...
if($mode=='SendAndRetrieveNew'){
	...
	//$id = $_GET['id'];
		...
		$delete = $chat->getMessage($_GET['msg_id']);
		$chat->deleteMessage($_GET['msg_id']);
}elseif($mode=='DeleteOneComment'){
		$chat->deleteMessageC($_GET['msg_id']);
}elseif($mode=='ReportOne'){
		$chat->reportMessage($_GET['msg_id']);
}elseif($mode=='ReportOneComment'){
		$chat->reportMessageC($_GET['msg_id']);
}elseif($mode=='More_messages'){
	$chat->middle_limit = $chat->middle_limit+3;
}elseif($mode=='MoreMy_messages' || $mode=='MoreMyP_messages'){
...
if($mode=='UpliftComment' || $mode=='ReportOneComment'){
	echo $chat->getComment($_GET['msg_id']);
}elseif($mode=='MoreMy_messages'){
	echo $chat->getMyMessages($_GET['current_user_id']);
}elseif($mode=='MoreMyP_messages'){
	echo $chat->getMyPrivateMessages($_GET['current_user_id']);
}elseif($mode=='Uplift'){
	echo  $chat->getMessage($_GET['msg_id']);
